<template>
  ...
  <el-col :span="12"><div class="grid-content bg-purple-light user" align="right">
       <el-menu class="el-menu-demo border-0" mode="horizontal">
         <el-menu-item index="1"><a href="/shangzhuang" class="text-decoration-none">上装系列</a></el-menu-item>
...
<div class="box">
  <div class="card ml-4 mb-3" style="width: 18rem" v-for="(item,index) in qun" :key="index">
...
export default {
    ...
    created(){
      ...
      this.$axios.get("http://localhost:8282/product/findAll").then(function(resp){
        console.log(resp);
        _this.qun=resp.data; 
        console.log(_this.qun);
        for(let i=0;i<_this.qun.length;i++)
      {
        if(_this.qun[i].kind=='下装' || _this.qun[i].kind=='上装')
        {
          _this.qun.splice(i,1);
          i--;
        }
